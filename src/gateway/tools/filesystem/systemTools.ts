...
import { finishTaskLanding } from '../git/landingService.js';
import { requireString, requireWorktreePath, resolvePathInsideRoot } from './pathSafety.js';
...
export async function readFileRaw(
  ...
  const fullPath = resolvePathInsideRoot(cwd, filePath);
...
export async function finishTask(worktreePath: string): Promise<{
  ...
  return finishTaskLanding(requireWorktreePath(worktreePath));
}
