...
async function main() {
  ...
  try {
    server = await startDevServer({
      command: 'npx tsx src/app/qa-fixture-server.ts',
      cwd: repoRoot,
      mirrorOutput: true,
    });
    ...
    const visual = await runQa(['tsx', 'src/app/visual-qa.ts', '--repo-root', repoRoot, '--shadow', '--no-server']);
    const e2e = await runQa(['tsx', 'src/app/e2e-qa.ts', '--repo-root', repoRoot, '--shadow', '--no-server']);
