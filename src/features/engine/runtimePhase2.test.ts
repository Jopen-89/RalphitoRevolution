...
import { ralphitoMigrations } from '../persistence/db/migrations.js';
...
test('resolveWriteScopeTargetsFromBeadFile colapsa a base paths reales', () => {
  ...
  try {
    ...
    writeFileSync(
      beadPath,
      '[WRITE_ONLY_GLOBS]: ["src/features/engine/**/*.ts", "src/features/engine/file.ts", "package.json"]\n',
      'utf8',
    );
    ...
    assert.deepEqual(
      ...
      ['package.json', path.join('src', 'features', 'engine')],
    );
    assert.deepEqual(targets[1]?.sourceGlobs, ['src/features/engine/**/*.ts', 'src/features/engine/file.ts']);
  } finally {
    ...
    assert.throws(
      () =>
        repository.acquireForSession({
          ...
          heartbeatAt,
          targets: [{ path: '/repo/src/features/engine/runtimeLockRepository.ts', pathKind: 'file' }],
        }),
