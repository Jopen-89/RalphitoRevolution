...
is_gateway_process_running() {
    pgrep -f "tsx src/app/server.ts" >/dev/null || pgrep -f "src/app/server.ts" >/dev/null
}

is_bot_running() {
    pgrep -f "node --import tsx src/interfaces/telegram/bot.ts" >/dev/null || pgrep -f "src/interfaces/telegram/bot.ts" >/dev/null
}
...
start_all() {
    ...
    if [ "$(get_gateway_http_code)" = "200" ] || is_gateway_process_running; then
        ...
        start_background_command "$GATEWAY_LOG" npx tsx src/app/server.ts
    fi
    ...
    if is_bot_running; then
        ...
        start_background_command "$BOT_LOG" node --import tsx src/interfaces/telegram/bot.ts
    fi
...
stop_all() {
    ...
    pkill -f "node --import tsx src/interfaces/telegram/bot.ts" || pkill -f "src/interfaces/telegram/bot.ts" || echo -e "${CYAN}El bot ya estaba detenido.${NC}"
    echo -e "${YELLOW}🛑 Deteniendo LLM Gateway...${NC}"
    pkill -f "tsx src/app/server.ts" || pkill -f "src/app/server.ts" || echo -e "${CYAN}El Gateway ya estaba detenido.${NC}"
...
join_chat() {
    ...
    run_repo_command npx tsx src/app/engine-status.ts table
    echo ""
...
live_monitor() {
    run_repo_command npx tsx src/app/live-monitor.ts
}
